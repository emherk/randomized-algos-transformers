...
from data_generator import AssociativeRecallData, Graph
from utils import get_qmc_seeds
...
class Loss(ABC):
    ...
    def __call__(self, params, rng, train=True):
        ...
        if self.cfg["data_pooling"] == "lp":
            if self.cfg["p"] >= 0:
                ...
                lp_loss = jnp.where(
                    max_loss > 0,
                    max_loss
                    * jnp.mean((loss / max_loss) ** self.cfg["p"])
                    ** (1 / self.cfg["p"]),
                    0,
                )
            else:
        ...
        if self.cfg["data_pooling"] == "powp":
            ...
            lp_loss = jnp.where(
                max_loss > 0, max_loss * jnp.mean((loss / max_loss) ** self.cfg["p"]), 0
            )
            return lp_loss, log_dict
            ...
            if self.cfg["p"] >= 0:
                return (
                    jnp.sum(
                        loss
                        * jax.nn.softmax(
                            loss / (loss.mean() + 0.00001) * (self.cfg["p"]), axis=0
                        )
                    ),
                    log_dict,
                )
...
class AssociativeRecallLoss(Loss):
    ...
    def __init__(self, model, cfg, foobar=False):
        ...
        self.data_generator = AssociativeRecallData(
    ...
    def get_loss_from_input(self, params, tokens, aux):
        ...
        if self.cfg["loss"] == "contrastive_hinge":
            prediction = (
                jnp.where(Y != 0, Y / jnp.linalg.norm(Y, axis=-1, keepdims=True), 0)
                @ prediction[-1]
            )
...
class ColoringLoss(Loss):
    ...
    def __init__(self, model, cfg):
        ...
        CANONICAL_CYCLE = jnp.eye(n, k=-1) + jnp.eye(cfg["num_vertex"], k=n - 1)
        ...
        if True:  # self.cfg["loss"] != "fractional_coloring":
            ...
            VALID_IDS_FOR_CANONICAL_CYCLE = jax.vmap(
                is_valid_3coloring, in_axes=(None, 0)
            )(CANONICAL_CYCLE, ALL_COLORS)
            self.VALID_COLORS_FOR_CANONICAL_CYCLE = ALL_COLORS[
                VALID_IDS_FOR_CANONICAL_CYCLE
            ]
            self.VALID_HOTS_FOR_CANONICAL_CYCLE = ALL_3_HOTS[
                VALID_IDS_FOR_CANONICAL_CYCLE
            ]
        # Permutation matrices for cycle symmetries
        S = np.eye(n - 1)[list(permutations(range(n - 1)))].reshape(
            (2, -1, n - 1, n - 1)
        )[0]
    ...
    def fractional_coloring(self, adjacency, logits, permutation):
        ...
        log_probs = jax.nn.log_softmax(logits)
        log_probs_neighbor = (
            permutation @ self.CANONICAL_CYCLE @ permutation.T
        ) @ log_probs

        log_prob_invalid_edge = log_probs_neighbor + log_probs
        loss = -jnp.log(
            1e-4 + 1.0 - jnp.exp(log_prob_invalid_edge).sum(-1)
        ).mean() + jnp.log(1e-4 + 1.0)
    ...
    def get_loss_from_input(self, params, graphs, adjacencies, permutations):
        # Computes the loss given model parameters and input data
        logits = self.model.apply(params, (graphs, adjacencies))
        ...
        if self.cfg["loss"] == "validity":
            ...
            loss = 1 - self.expected_validity(adjacencies, logits, permutations)
    ...
    def get_loss(self, params, mat_seed, rng_seed):
        ...
        graphs, adjacencies, permutations = self.graph_generator.sample(
            mat_seed, rng_seed
        )
